...
ANTS = 84              # Number of Ants to spawn
    ...
    def update(self, dt, food_count, food_coount_previous, fpschecker, efficiency):  # behavior
        ...
        if self.mode == 0 and self.pos.distance_to(self.nest) > 21:
            ...
            if left_GA_result == foodColor and right_GA_result != foodColor :
                ...
                if self.drawSurf.get_rect().collidepoint(mid_sens):  # Check if ant is on food
            ...
            if random.random() < self.death_probability:  # Check if the ant dies
                self.enable = 0
                self.mode = 3
                self.rest = 5000000000000000000  # Optional: Add a rest period for the ant after death
                self.energy_use = 0  # Set energy_use to 0 after death
                self.fatigue = 0  # Reset fatigue
...
class Food(pg.sprite.Sprite):
    def __init__(self, pos):
        super().__init__()
        self.pos = pos
        self.size = 16  # 食べ物の初期サイズ
        self.image = pg.Surface((self.size, self.size))
        self.image.fill(0)  # 黒で塗り潰す
        self.image.set_colorkey(0)  # 黒を透明として設定する
        pg.draw.circle(self.image, [20, 150, 2], [self.size // 2, self.size // 2], self.size // 4)
...
def main():
    parser = argparse.ArgumentParser(description="NAnts simulation with threshold modes.")
    parser.add_argument("--threshold_mode", choices=["fixed", "random"], default="fixed", help="Threshold selection mode for ants.")
    parser.add_argument("--savefn", default = "result.txt", help = "filename to save the result.")
    ...
    fpsChecker = 0
    for n in range(ANTS):
        workers.add(Ant(screen, nest, pheroLayer, foodList, random.random(), 10, args.threshold_mode))
    food_count = [0]
...
#        pheroLayer.img_array[170:182,0:80] = (50,50,50)  # wall
        ...
        for food in foodList:
            food.image = pg.Surface((food.size, food.size))
            food.image.fill(0)
        ...
        if fpsChecker % 100 == 0:
            ...
            efficiency_ratio = count / ANTS
            print("efficiency ratio: ", efficiency_ratio)

            with open (args.savefn, "a") as f:
                f.write(f"{fpsChecker}, {food_count_previous}\n")
        fpsChecker += 1
